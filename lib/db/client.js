  ...
  var _ref = _asyncToGenerator(function* (server, database) {
    ...
    try {
      ...
      if (server.config.ssh && !server.sshTunnel) {
        ...
        var _server$sshTunnel$add = server.sshTunnel.address();
      ...
      var _ref2 = yield Promise.all([driver(server, database), handleSSHError(server.sshTunnel)]),
          _ref3 = _slicedToArray(_ref2, 1);

      const connection = _ref3[0];
...
let getQuerySelectTop = (() => {
  var _ref4 = _asyncToGenerator(function* (server, database, table, schema, limit) {
...
let getTableUpdateScript = (() => {
  var _ref7 = _asyncToGenerator(function* (server, database, table, schema) {
    const columnNames = yield getTableColumnNames(server, database, table, schema);
...
let getTableColumnNames = (() => {
  var _ref8 = _asyncToGenerator(function* (server, database, table, schema) {
    checkIsConnected(server, database);
...
let loadConfigLimit = (() => {
  var _ref9 = _asyncToGenerator(function* () {
    if (limitSelect === null) {
      var _ref10 = yield config.get();

      const limitQueryDefaultSelectTop = _ref10.limitQueryDefaultSelectTop;
...
var _bigquery = require('./clients/bigquery');

var _bigquery2 = _interopRequireDefault(_bigquery);
...
function createConnection(server, database) {
  ...
  if (server.config.name == 'bigquery' || server.config.client === 'bigquery') {
    ...
    return (0, _bigquery2.default)(database);
  }
  /**
   * Database public API
   */
